...
public class PropietarioControl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch(accion){
        case "reportes":
        	rd = request.getRequestDispatcher("faces/Reporte.xhtml");
        ...
        case "registrarlugar":
        	...
        	if (registro==1){
        	rd = request.getRequestDispatcher("propietario.jsp?msg=El lugar se ha registrado correctamente");
    		}
        	else{
        		rd = request.getRequestDispatcher("propietario.jsp?msg=Registro invalido, hay otro lugar en sus coordenadas");	
        	}
        	rd.forward(request, response);
    		break;
        case "actualizar":
        	...
        	session.setAttribute("persona", propietario);
        	rd = request.getRequestDispatcher("propietario.jsp?msg=Datos actualizados correctamente");
    		rd.forward(request, response);
    		break;
        	...
        	try {
				...
				if(lugares.isEmpty()){
					PrintWriter out=response.getWriter();
					   ...
					   rd = request.getRequestDispatcher("propietario.jsp?msg=No tiene lugares registrados");
			    		rd.forward(request, response);
				}
				else{
        ...
        case "eliminar":
        	...
        	try {
				...
				rd = request.getRequestDispatcher("propietario.jsp?msg=El lugar se ha eliminado.");
	    		rd.forward(request, response);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
